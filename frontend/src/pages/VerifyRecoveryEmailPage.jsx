...
export default function VerifyRecoveryEmailPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const res = await API.get("/auth/verify-recovery-email", { params: { token } });
        if (res.data?.message) {
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl shadow-black/30 text-slate-100">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-semibold">Recovery Email Verification</h1>
          <p className="text-sm text-slate-400">
            {status === "loading" && "Please wait while we verify your recovery email..."}
