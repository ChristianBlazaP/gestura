...
// File upload handlers (expect multer to populate req.file)
...
exports.uploadCoverFile = async (req, res) => {
  if (!req.file) return res.status(400).json({ error: "No file uploaded" });
  const url = `/uploads/${req.file.filename}`;
  try {
    await query(`UPDATE users SET cover_url = ? WHERE id = ?`, [url, req.user.id]);
    return res.json({ url });
  } catch (err) {
...
exports.uploadPhotoFile = async (req, res) => {
  ...
  try {
    await query(`INSERT INTO photos (user_id, url, caption) VALUES (?, ?, ?)`, [req.user.id, url, caption]);
    return res.json({ message: "Photo added", url });
  } catch (err) {
...
// Public/other user's photos
exports.listPhotosByUser = async (req, res) => {
  ...
  try {
    ...
    console.error("PROFILE list photos by user error:", err);
    return res.status(500).json({ error: "Database error (photos by user)" });
  }
};
...
exports.addPhoto = async (req, res) => {
  ...
  try {
    await query(`INSERT INTO photos (user_id, url, caption) VALUES (?, ?, ?)`, [
      userId,
      url.trim(),
      caption.trim(),
    ]);
...
exports.toggleLike = async (req, res) => {
  ...
  try {
    const [exists] = await query(`SELECT id FROM photo_likes WHERE photo_id = ? AND user_id = ?`, [
      photoId,
      userId,
    ]);
    if (exists.length) {
